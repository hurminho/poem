  ...
  return (
    <div className="poem-page">
      <div className="mx-auto max-w-2xl px-6 pt-8 flex items-center justify-between gap-3 text-xs text-text-secondary">
        <Link href={`/books/${book.id}`} className="hover:text-text-primary transition-colors truncate">
          ← {book.title}
        </Link>
        <ReaderThemeToggle />
      </div>
      ...
      <PoemReader
        ...
        actions={<ReaderActions reflectionAnchor="#reflection-section" />}
      />
      ...
      <nav className="mx-auto max-w-2xl px-6 pb-8 -mt-6 flex items-center justify-between">
        {prevIdx !== null ? (
          ...
          >
            <ChevronLeft className="size-4" /> 이전 시
          </Link>
        ) : (
          <span />
        )}
        {nextIdx !== null ? (
          <Link
            ...
            className="inline-flex items-center gap-1 text-sm text-text-secondary hover:text-text-primary"
          >
            다음 시 <ChevronRight className="size-4" />
          </Link>
        ) : (
